...
(() => {
  ...
  const stylesheet =
    ...
    // compact: thumbnail-sized slots (< ~80px) have no room for the caption —
    // show just the icon so the empty state never overflows its box.
    ':host([compact]) .empty{padding:4px;gap:0}' +
    ':host([compact]) .empty .cap,:host([compact]) .empty .sub{display:none}' +
    ':host([compact]) .empty svg{width:18px;height:18px}' +
