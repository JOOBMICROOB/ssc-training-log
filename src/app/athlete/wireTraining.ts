...
function setRow(ex: SessionExercise, ei: number, st: LoggedSet, si: number, locked: boolean): string {
  ...
  return `<div style="margin-left:12px;padding:7px 10px 8px 12px;border-left:2px solid rgba(89,128,166,.45);">
    <div style="display:flex;align-items:center;gap:8px;">
      <span style="flex:0 0 auto;width:44px;font:600 12px/1 'Barlow Condensed',sans-serif;letter-spacing:.1em;color:rgb(107,116,128);">SET ${si + 1}</span>
      <span style="flex:1 1 0;font:400 11.5px/1 Barlow,sans-serif;color:rgb(95,104,115);">${st.targetReps} reps${targetShort(st) ? ` @ ${targetShort(st)}` : ""}${st.targetSuggest && !st.targetLoad ? ` · <span style="color:#41617f;font-weight:600;">try ${st.targetSuggest} kg</span>` : ""}${st.backoffPct != null && si >= 1 && !st.targetLoad ? ` · <span style="color:#41617f;font-weight:600;">−${st.backoffPct}% of set 1</span>` : ""}${st.fixedLoad ? ' · <span style="color:#41617f;font-weight:600;">FIXED</span>' : ""}${st.failed ? ' · <span style="color:#b45454;font-weight:600;">FAILED</span>' : ""}</span>
    </div>
