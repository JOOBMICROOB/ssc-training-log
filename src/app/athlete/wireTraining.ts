    ...
    .map((d) => {
      ...
      const style = active
        ...
        : d.rest
          ...
          : "border:1px solid rgba(29,31,32,.14);background:rgba(89,128,166,.12);color:rgb(60,69,79);";
...
function setRow(ex: SessionExercise, ei: number, st: LoggedSet, si: number, locked: boolean): string {
  ...
  const step = (label: string, attr: string) =>
    `<button ${attr} ${dis} style="flex:0 0 auto;width:32px;height:36px;border:1px solid rgba(29,31,32,.14);border-radius:9px;background:#fff;color:#41617f;font:600 17px/1 'Barlow Condensed',sans-serif;cursor:pointer;">${label}</button>`;
  ...
  const rpeRow = st.requiresRpe
    ? `<div style="display:flex;align-items:center;gap:8px;margin-top:6px;">
        ...
        <input type="range" data-rpe="${key}" ${dis} min="5" max="10" step="0.5" value="${st.rpe ?? 7.5}" style="flex:1 1 0;height:18px;accent-color:#41617f;">
  ...
  return `<div style="margin-left:12px;padding:7px 10px 8px 12px;border-left:2px solid rgba(89,128,166,.45);">
    <div style="display:flex;align-items:center;gap:8px;">
      <span style="flex:0 0 auto;width:44px;font:600 12px/1 'Barlow Condensed',sans-serif;letter-spacing:.1em;color:rgb(107,116,128);">SET ${si + 1}</span>
      <span style="flex:1 1 0;font:400 11.5px/1 Barlow,sans-serif;color:rgb(95,104,115);">${st.targetReps} reps${targetShort(st) ? ` @ ${targetShort(st)}` : ""}${st.targetSuggest && !st.targetLoad ? ` · <span style="color:#41617f;font-weight:600;">try ${st.targetSuggest} kg</span>` : ""}${st.backoffPct != null && si === 0 ? ' · <span style="color:#41617f;font-weight:600;">TOP SET</span>' : ""}${st.backoffPct != null && si >= 1 && !st.targetLoad ? ` · <span style="color:#41617f;font-weight:600;">−${st.backoffPct}% of set 1</span>` : ""}${st.fixedLoad ? ' · <span style="color:#41617f;font-weight:600;">FIXED</span>' : ""}${st.failed ? ' · <span style="color:#b45454;font-weight:600;">FAILED</span>' : ""}</span>
    </div>
...
function exerciseBlock(ex: SessionExercise, ei: number, expanded: boolean, locked: boolean): string {
  ...
  const header = `<button data-ex="${ei}" style="width:100%;display:flex;align-items:center;gap:10px;text-align:left;padding:11px 12px;cursor:pointer;border-radius:12px;backdrop-filter:blur(16px);box-shadow:rgba(20,36,52,.07) 0px 4px 14px;${headStyle}">
    ...
    <div style="flex:0 0 auto;text-align:right;">
      ...
      <div style="margin-top:2px;font:400 9.5px/1 Barlow,sans-serif;letter-spacing:.08em;color:rgb(65,97,128);">${ex.loggedCount} / ${ex.setCount} LOGGED</div>
    </div>
  ...
  function draw() {
    ...
    const sessions = week
      .map((d) => {
        ...
        return `<button data-cal="${d.date}" style="width:100%;display:flex;align-items:center;gap:12px;text-align:left;padding:11px 12px;margin-top:6px;border-radius:14px;cursor:pointer;border:1px solid ${sel ? "rgb(89,128,166)" : "rgba(29,31,32,.10)"};background:${sel ? "rgba(89,128,166,.12)" : "rgba(255,255,255,.7)"};">
          <span style="flex:0 0 auto;width:30px;text-align:center;font:600 16px/1 'Barlow Condensed',sans-serif;letter-spacing:.04em;color:#41617f;">${d.sessionLabel}</span>
          ...
          <span style="flex:0 0 auto;font:600 9.5px/1 'Barlow Condensed',sans-serif;letter-spacing:.1em;color:#41617f;">${status}</span>
        </button>`;
      })
      .join("");
    root.innerHTML = `<div style="width:344px;max-width:96vw;max-height:calc(100dvh - 32px);overflow-y:auto;background:#f4f8fc;border:1px solid rgba(29,31,32,.12);border-radius:20px;box-shadow:0 24px 60px rgba(9,17,28,.4);padding:16px;">
      <div style="display:flex;align-items:center;gap:8px;">
        <button data-cal-prev style="width:34px;height:34px;border:1px solid rgba(29,31,32,.14);border-radius:9px;background:#fff;color:#41617f;cursor:pointer;">‹</button>
        ...
        <button data-cal-next style="width:34px;height:34px;border:1px solid rgba(29,31,32,.14);border-radius:9px;background:#fff;color:#41617f;cursor:pointer;">›</button>
        <button data-cal-close style="width:34px;height:34px;border:0;background:transparent;color:#41617f;font-size:16px;cursor:pointer;">✕</button>
      </div>
      ...
      <div style="display:flex;gap:14px;margin-top:12px;font:400 9px/1 Barlow,sans-serif;letter-spacing:.06em;color:#6b7480;">
        ...
        <span><span style="display:inline-block;width:9px;height:9px;border-radius:3px;background:rgba(89,128,166,.4);margin-right:5px;vertical-align:-1px;"></span>LOGGED</span>
      </div>
      <button data-cal="${today}" style="width:100%;margin-top:12px;padding:11px;border:1px solid rgb(89,128,166);border-radius:12px;background:rgba(89,128,166,.12);color:#1d2d3d;font:600 12px/1 'Barlow Condensed',sans-serif;letter-spacing:.12em;cursor:pointer;">GO TO TODAY'S SESSION</button>
...
export function wireTraining(host: HTMLElement, athleteId: string): () => void {
  ...
  shareBtn.style.cssText =
    "width:100%;margin-top:8px;padding:13px;border:1px solid rgb(89,128,166);border-radius:14px;background:rgba(89,128,166,.12);color:rgb(29,45,61);font:600 13px/1 'Barlow Condensed',sans-serif;letter-spacing:.12em;cursor:pointer;display:none;";
